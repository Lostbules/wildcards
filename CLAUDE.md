...
| 文件 | 角色 | 规则 |
|------|------|------|
| `NSFW_sex_categories/*.txt` | **权威源** | 所有 NSFW 姿势编辑在此进行 |
| `XL-pose_NSFW_sex.txt` | 派生 | **禁止直接编辑**，从子文件夹重建 |
| `XL-pose_merged.txt` | 派生 | **禁止直接编辑**，从三个源文件 cat 重建 |
| `XL-pose_SFW_clean.txt` | 独立源 | 可直接编辑（无子文件夹） |
| `XL-pose_NSFW_solo-nude.txt` | 独立源 | 可直接编辑（无子文件夹） |
| `characters_categories/clothed/*.txt` | **权威源** | 所有带衣物标签的条目 |
| `characters_categories/bare/*.txt` | **权威源** | 所有去衣条目 |
| `characters_categories/*.txt`（4系列） | 派生 | **禁止直接编辑**，从 clothed/ + bare/ 去重重建 |
| `characters_curated*.txt` | 派生 | **禁止直接编辑**，从 4 系列文件去重重建 |

**为什么禁止编辑主文件：** 主文件是 `set` 去重产物，一条主文件条目可能对应多个子文件位置。这个映射是有损的——主文件中找不到对应条目时，不知道应该改哪个子文件。反向同步不可行。**宁丢修改，不反向污染。**

**重复规则**：子文件夹内同一条目允许在不同分类文件中重复出现（一条目可归属多个类别）；主 txt 文件不允许重复，`sorted(set(lines))` 自动去重。

**如果主文件被意外修改**：从子文件重建（见 §6），主文件上的任何手动修改被丢弃。
...
- 分支：`master`
- 推送流程：`git add -A && git commit -m "..." && git push`
...
```bash
# NSFW 姿势：编辑权威源 → 同步主文件 → 刷新索引
vim NSFW_sex_categories/XX_xxx.txt
python program/nsfw_xref.py --check        # 跨文件重复检查（涉及多分类共有条目时）
python program/verify_sync.py --fix        # 重建主文件 + merged
python program/pose_index.py               # 刷新姿势索引

# 角色：编辑权威源 → 重建派生文件 → 重建索引
vim characters_categories/clothed/xxx.txt  # 或 bare/xxx.txt
python program/rebuild_characters.py --apply
python program/series_index.py             # 增删后必须
...
- `grep -w` — 精确匹配单词（`changing` ≠ `hanging`）；`grep -c` — 先统计总量再决定是否 `grep -n` 看详情
- `grep -iE` — 多模式正则；结果出来人工确认，不看计数就下结论（`saki` 会匹配到 `misaki`）
- 文件中有 `\`（如 LoRA 路径），先用 `tr '\\' '/'` 转换再处理；`sed` 中写 `\\` 在 Windows bash 下容易出错，改用 `tr` + `sed` 分步
...
- **同文件内多处编辑：必须从高行号到低行号处理**。删 L12 再删 L10 不会影响 L10 的行号；反过来会导致原 L12 变成 L11。示例：`sed -i.bak '12d;10d' file.txt`（不是 `10d;12d`）
- 适用范围：删除行、替换行内容、任何会让后续行号漂移的操作
- **跨文件编辑：先改子文件夹（权威源），再重建主文件**。主文件永远不手改，不存在行号漂移问题
- 每轮前创建 `.bak`：`sed -i.bak '12d;10d' file.txt`，事后 `grep` 验证目标清零

### 备份管理

- 命名：`[原文件名]_[描述]_[YYYYMMDD-HHMM].bak`，描述用中文标注阶段（`原始备份`、`第一轮清理` 等）
- 时间戳使用文件修改时间（UTC，精确到分钟）；备份统一存放 `backups/`
- `sed -i.bak` 多次执行产生 `.bak`、`.bak2`、`.bak3`……根据时间戳和文件大小还原阶段
...
**阶段 2 — 审查标记**：按条目数从多到少分批呈现，`[KEEP]` / `[DEL]` 标记。大题材（10+条目）逐题材呈现；中型（3-9）合并一表；小题材（1-2）用户一次性点名保留，其余默认删除。关键原则：属性标签（hair/eyes/clothing）不是题材；孤立无逗号行是遗留标题；小众题材向删除倾斜。
...
适用于无 `name, series` 结构的平铺标签文件。**多标签分类**（一条目可归入多个类别）：

```bash
awk -F',' '{
  if ($0 ~ /keyword_a/)     print >> "cat_a.txt"
  if ($0 ~ /keyword_b/)     print >> "cat_b.txt"
  if (!($0 ~ /keyword_a|keyword_b/)) print >> "other.txt"
}' file.txt
```
...
- 标签必须经过双重验证：① 优先查标准库（见「Danbooru 标签参考」）；② 其次从已有文件中验证存在（`grep -c 'tag' file.txt`）。不凭空创造
- 不同条目间的变体比例保持均衡
...
4. **分层创建**：第一层 Danbooru 经典场景（填补缺失行/列）→ 第二层 场景×属性混搭（填补空白格）→ 第三层 角度/表情等细节变体
...
**标签冗余**：避免 `sleeping, asleep`；`train interior, train`；`changing room, clothing store, fitting room`。

**拼写检查**：`speed tines`→`speed lines`，`shinny`→`shiny`。

**表现力**：纯公式化变体（仅追加 1 个标签如 `phone`）视为弱表现力，建议重写。

**标签降级**：统一 body part 尺寸级别时，`gigantic/huge/massive/giant/enormous/monster` → 降为 `large` 或不带尺寸修饰。`large` 为允许的最大级别；`tiny/small` 不降级。同一行同时出现 `large` + 更大同义词时，删大的保留 `large`。`huge` 可能修饰非目标对象（`huge ass`、`huge ahoge`），逐条确认。
...
标准标签库位于 tag-crafter skill（目录树见 §9），按类别分 34 个 CSV 文件。**CSV 使用下划线格式**（`open_clothes`），wildcard 使用空格格式（`open clothes`），校验时需转换。

```bash
# 单标签校验（下划线格式）
grep -r "^tag_name," ~/.claude/skills/tag-crafter/tag-library/categorized/

# 批量校验（空格格式输入，自动转下划线）
for tag in $(cat _planned_tags.txt); do
  underscore=$(echo "$tag" | tr ' ' '_')
  count=$(grep -rlc "^$underscore," ~/.claude/skills/tag-crafter/tag-library/categorized/)
  echo "$tag → $count"
done
```

CSV 格式：`tag_name,0,post_count,aliases`。常见非标准→标准对照见 `~/.claude/skills/tag-crafter/references/standardization-fixups.md`。
...
详细构图配方见 `~/.claude/skills/tag-crafter/references/composition-recipes.md`。
...
**NSFW 姿势：** `编辑权威源(高→低行号) → verify_sync.py --fix → pose_index.py`
**角色：** `编辑权威源(高→低行号) → rebuild_characters.py --apply → series_index.py`

跨文件编辑先改子文件夹，再重建主文件。**重建前必须询问用户确认，不得自动同步。**

### 重建命令速查

| 场景 | dry-run | 执行（写文件） |
|------|---------|---------------|
| NSFW 姿势同步 | `python program/verify_sync.py` | `python program/verify_sync.py --fix` |
| 角色重建 | `python program/rebuild_characters.py` | `python program/rebuild_characters.py --apply` |
| 姿势索引 | — | `python program/pose_index.py` |
| 角色索引 | — | `python program/series_index.py` |

`verify_sync.py` 额外检查：重复标签、括号不匹配、双逗号、LoRA 前缀损坏。孤儿条目保存到 `_orphans.txt` 供人工审查。
...
权威层级和编辑流程见 §1 和 §6。本节仅覆盖角色特有的操作规范。
...
除非用户有特殊要求，否则 **先做 clothed 版本，bare 由 clothed 删除衣物/装备/手持物标签得到**，只保留发色、瞳色、身材、丝袜等身体描述标签。避免凭空构造 bare 条目导致外貌标签与 clothed 不一致。

```
# 1. 在 clothed/ 对应系列文件末尾追加完整条目（含衣物装备）
character_name, series_name, tag1, tag2, ..., clothing_tag, gear_tag

# 2. 从 clothed 条目删除衣物/装备标签，追加到 bare/ 对应文件
character_name, series_name, tag1, tag2, ...  # 仅保留发色瞳色身材丝袜等身体描述
```
...
│   └── full-prompt.txt
└── tag-library/
    └── categorized/                 # 34 个 CSV（下划线格式），标签校验见 §5
```

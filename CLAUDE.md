...
# 2. 检查跨文件重复（编辑涉及多分类共有条目时必须）
python program/nsfw_xref.py --check
# 3. 同步陈旧重复（若有）
python program/nsfw_xref.py --sync --auto
# 4. 同步主文件 + merged
...
# 3. 重建角色索引（增删后必须）
python program/series_index.py
```
...
- 标签必须经过双重验证：① 优先查标准库（见「Danbooru 标签参考」校验命令）；② 其次从已有文件中验证存在（`grep -c 'tag' file.txt`）。不凭空创造
- 不同条目间的变体比例保持均衡
...
标准标签库位于 tag-crafter skill 的 `tag-library/categorized/` 目录（`~/.claude/skills/tag-crafter/tag-library/categorized/`），按类别分 34 个 CSV 文件。**CSV 使用下划线格式**（如 `open_clothes`），wildcard 文件使用空格格式（`open clothes`），校验时需转换。
...
CSV 格式：`tag_name,0,post_count,aliases`

常见的非标准→标准对照见 `~/.claude/skills/tag-crafter/references/standardization-fixups.md`。
...
3. python program/series_index.py                  # 重建角色索引（增删后必须）
```
...
│   ├── wildcard-entry.txt
│   └── full-prompt.txt
└── tag-library/                     # 标准标签库（权威源，校验方法见 §5）
    └── categorized/                 # 按类别分 34 个 CSV（下划线格式）
```

标签校验方法见 §5「Danbooru 标签参考」。
